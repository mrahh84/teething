...
# --- User Configuration ---
# Use relative paths for input and output
...
csv_output_dir = os.path.join(current_dir, 'output')
...
def clean_this_df(raw_df):
    ...
    narrow_df['problems'] = narrow_df['STAND-UP'].fillna(0) + narrow_df['RETURNED TO WORKSTATION ON TIME AFTER LUNCH'].fillna(0)
...
def create_visualizations_from_results(results_df, comprehensive_report):
    """Create all visualizations using Plotly."""
    ...
    try:
        ...
        if top_chart_success and infractions_chart_success:
...
def create_plotly_distribution(results_df, all_employees):
    ...
    try:
        ...
        print(f"Distribution graph saved to {os.path.join(output_dir, 'employee_distribution.html')}")
...
def main():
    ...
    comprehensive_report['Total Individual Issues'] = comprehensive_report['Stand-up Issues'] + comprehensive_report['Return from Lunch Issues']
    ...
    # Save the problematic analysis results
    ...
    create_plotly_distribution(results_df, unique_employees)
