...
@reporting_required  # Changed from admin_required for testing
@extend_schema(exclude=True)
...
# Test view without authentication for styling verification
def performance_dashboard_test(request):
    """Test view for performance dashboard styling - no authentication required."""
    # Mock data for testing
    mock_context = {
        'page_title': 'Performance Dashboard Test',
        'active_tab': 'performance_dashboard',
        'real_time_status': {
            'current_clocked_in': 45,
            'expected_attendance': 52,
            'attendance_status': 'normal',
            'working_hours_info': {
                'status': 'open',
                'message': 'Office is open - Normal working hours'
            }
        },
        'attendance_trends': {
            'summary': {
        ...
        'department_heatmap': {
            'departments': [
                {
                    'department_name': 'Digitization Tech',
                    'performance_score': 89.2,
                    'performance_tier': 'excellent',
                    'trend': 'improving'
                },
                {
                    'department_name': 'Conservation',
        ...
        'enhanced_system_metrics': {
            'summary': {
                'avg_daily_events': 156,
                'total_events_period': 1092
            }
        },
        'cache_stats': {
            'calculated_hit_rate': 92.3,
            'total_keys': 45
        },
        'opening_hours_info': {
            'upcoming_holidays': [
                {'date': '2025-08-25', 'name': 'Kadooment Day', 'days_until': 9, 'is_today': False},
                {'date': '2025-09-01', 'name': 'Labour Day', 'days_until': 16, 'is_today': False}
            ],
            'working_hours_summary': {
                'working_days_per_week': 5,
                'total_working_hours_per_week': 40
            },
            'next_working_day': {
                'day': 'Monday',
                'start_time': '09:00',
        ...
        'all_departments_working_hours': {
            'Digitization Tech': {
                'code': 'DT',
                'description': 'Digital transformation and technology',
                'working_hours': {'working_days_per_week': 5, 'total_working_hours_per_week': 40},
                'schedule': {
        ...
        'performance_recommendations': [
            {
                ...
                'suggestion': 'Current average: 87.5%. System is performing well.'
            }
        ],
        'last_updated': django_timezone.now(),
        'chart_data': {
            'attendance_trends_labels': json.dumps(['Mon', 'Tue', 'Wed', 'Thu', 'Fri']),
            'attendance_trends_data': json.dumps([85, 88, 90, 87, 89]),
        }
    }
    
    return render(request, 'reports/performance_dashboard.html', mock_context)
